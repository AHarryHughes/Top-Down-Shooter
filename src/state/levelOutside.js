...
export default class LevelOutside extends Phaser.State {
    ...
    create(game) {
        ...
        this.shotgunEnemyBullets = null;

        //*******************************************************************************TILE POSITIONS*************************
        this.map = game.add.tilemap("outside");
        this.map.addTilesetImage('outside-tileset', 'outside-tileset');

        //*******************************************************************************MAP CREATION::BASE*******************************
        let layer = this.map.createLayer('Base');
        layer.resizeWorld();

        //*****************************************************************************MAP CREATION::COLLISION*********
        let collisionLayer = this.map.createLayer('Collision');
        ...
        let foregroundCollisionLayer = this.map.createLayer('ForegroundObjects');
        game.foregroundCollisionLayer = foregroundCollisionLayer;
        foregroundCollisionLayer.visible = true;
        this.map.setCollisionByExclusion([], true, this.foregroundCollisionLayer);
        ...
        this.levelText = this.add.text(0, 30, "level", { fontSize: '32px', fill: '#fff' });
    ...
    update(game) {
        ...
        this.levelText.text = 'Player Level: ' + this.playerLevel;
        //*******************************************************************************COLLIDERS SET*******************************
        ...
        enemies.forEachAlive(function (enemies) {
            ...
            this.chasePlayer(this.player, enemies, game);
        });
    ...
    enemyShot(bullets, enemies) {
        enemies.damage(20);
        bullets.kill();
    }

    //*******************************************************************************INFLICTS DAMAGE TO ENEMY*******************************
    playerShot(player, shotgunEnemyBullets) {
